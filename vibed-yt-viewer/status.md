...
- **Robots.txt – Disallow All Crawlers**
  ...
  - **Policy:**
    ...
    Disallow: /
    ```
  - Purpose: Prevent indexing during development/private usage. Update when ready for SEO.

- **Markdown Export on Detail View**
  - Subtle top-center button added to `src/components/video-detail-client-view.tsx`.
  - Exports a `.md` file named `Title-VideoID.md` (sanitized).
  - Includes YAML frontmatter: Title, VideoID, Channel, ImportanceRating, dates (Created/Updated/Published), genre, tags (Hashtags), relations (Persons, Companies, Indicators, Trends, Institutions), InvestableAssets, TickerSymbol, EventsFairs, DOIs, PrimarySources, RelatedURLs, BookMediaRecommendations, Sentiment, SentimentReason.
  - Body sections include: Video URL, Actionable Advice, TLDR, Main Summary, Detailed Narrative Flow, Memorable Quotes/Takeaways, Key Numbers & Data, Key Examples, Personal Comment, Associated URLs, Original Description, Technical Terms, Primary Sources, and Transcript if present.
  - Placement fix: Export button now rendered in right sidebar directly under the Channel card in `src/app/video/[videoId]/VideoDetailPageContent.tsx`.
  - Robustness: Added Safari/legacy fallback (opens blob URL in new tab if `download` attribute unsupported) and delayed URL revocation.
...
- **updateVideo and deleteVideo now use the robust v1 endpoint**
  - Uses project and table NAMES, not IDs
  - Auth via `xc-token` header
  - No `xc-project` header needed
  - Config vars: `NOCODB_PROJECT_ID` (should be project name), `NOCODB_TABLE_ID` (should be table name), `NC_URL`, `NC_TOKEN`

## In Progress

- Further API error debugging if any
