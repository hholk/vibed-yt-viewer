...
export function VideoDetailPageContent({
  ...
  // Handler to save the Personal Note (PersonalComment)
  // This function updates the PersonalComment field of the current video
  // It uses the updateVideo function to save the changes to the server
  // It also updates the local state to reflect the saved note
  // Handler to save the Personal Note (PersonalComment)
  // Uses updateVideo to persist the note to the server and updates local state
  const handleSaveComment = async () => {
    if (!currentVideo?.Id) return;
    setIsSaving(true);
    setSaveError(null);
    try {
      // Create a payload with the updated PersonalComment
      const updatedFields = { PersonalComment: personalComment };
      // Call updateVideo with the correct payload for PersonalComment
      await updateVideo(currentVideo.Id, updatedFields);
      // Update local state to reflect the saved note
  ...
  // Handler to delete the current video
  const handleDeleteVideo = async () => {
    ...
    setSaveError(null);
    try {
      // Call deleteVideo with the correct ID
  ...
  // Convert values to simple YAML lines
  const toYAML = useCallback((key: string, value: unknown) => {
    ...
    if (Array.isArray(value)) {
      ...
      const items = value
        .map((v) => {
          ...
          if (typeof v === 'object' && v && 'Title' in v) return `  - ${(v as any).Title ?? ''}`;
          if (typeof v === 'object' && v && 'name' in v) return `  - ${(v as any).name ?? ''}`;
          return `  - ${String(v)}`;
        })
        .join('\n');
      return `${key}:\n${items}\n`;
  ...
  const buildMarkdown = useCallback((v: Video) => {
    const frontmatter =
      '---\n' +
      toYAML('Title', v.Title ?? '') +
      toYAML('VideoID', v.VideoID ?? '') +
      toYAML('Channel', v.Channel ?? '') +
      toYAML('ImportanceRating', v.ImportanceRating ?? '') +
      toYAML('CreatedAt', v.CreatedAt ?? null) +
      toYAML('UpdatedAt', v.UpdatedAt ?? null) +
      toYAML('PublishedAt', v.PublishedAt ?? null) +
      toYAML('VideoGenre', v.VideoGenre ?? '') +
      toYAML('Hashtags', v.Hashtags ?? []) +
      toYAML('Persons', v.Persons ?? []) +
      toYAML('Companies', v.Companies ?? []) +
      toYAML('Indicators', v.Indicators ?? []) +
      toYAML('Trends', v.Trends ?? []) +
      toYAML('InvestableAssets', v.InvestableAssets ?? []) +
      toYAML('TickerSymbol', (v as any).TickerSymbol ?? '') +
      toYAML('Institutions', v.Institutions ?? []) +
      toYAML('EventsFairs', v.EventsFairs ?? []) +
      toYAML('DOIs', v.DOIs ?? []) +
      toYAML('PrimarySources', v.PrimarySources ?? []) +
      toYAML('RelatedURLs', v.RelatedURLs ?? []) +
      toYAML('BookMediaRecommendations', (v as any).BookMediaRecommendations ?? []) +
      toYAML('Sentiment', v.Sentiment ?? '') +
      toYAML('SentimentReason', v.SentimentReason ?? '') +
      '---\n\n';
    ...
    const stringifyKeyNumbers = () => {
      const val = (v as any).KeyNumbersData as unknown;
      if (val == null) return '';
    ...
    const body =
      section('Video URL', v.URL ?? (v.RelatedURLs && v.RelatedURLs[0]) ?? '') +
      section('Actionable Advice', v.ActionableAdvice ?? '') +
      section('TLDR', v.TLDR ?? '') +
      section('Main Summary', v.MainSummary ?? '') +
      section('Detailed Narrative Flow', v.DetailedNarrativeFlow ?? '') +
      list('Memorable Quotes', v.MemorableQuotes ?? []) +
      list('Memorable Takeaways', v.MemorableTakeaways ?? []) +
      section('Key Numbers & Data', stringifyKeyNumbers()) +
      list('Key Examples', v.KeyExamples ?? []) +
      section('Personal Comment', v.PersonalComment ?? '') +
      list('Associated URLs', v.RelatedURLs ?? []) +
      section('Original Description', v.Description ?? '') +
      list('Technical Terms', v.TechnicalTerms ?? []) +
      list('Primary Sources', v.PrimarySources ?? []) +
      (v.FullTranscript || v.Transcript ? `## ${v.FullTranscript ? 'Full Transcript' : 'Transcript'}\n\n${(v.FullTranscript || v.Transcript) as string}\n\n` : '');
  ...
  const handleExportMarkdown = useCallback(() => {
    try {
      const md = buildMarkdown(currentVideo);
      const title = sanitizeFileName(currentVideo.Title ?? 'Video');
      const videoId = sanitizeFileName(currentVideo.VideoID ?? 'Unknown');
      ...
      // Some Safari versions don't support programmatic click with download
      ...
      a.remove();
      // Revoke after a tick to ensure blob is consumed
      setTimeout(() => URL.revokeObjectURL(url), 0);
    } catch (e) {
      console.error('Markdown export failed', e);
    }
  }, [buildMarkdown, currentVideo, sanitizeFileName]);
      ...
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 space-y-6">
            ...
            {/* Export Markdown card (directly under Channel) */}
            <div className="p-4 bg-neutral-800 rounded-lg shadow">
              <h3 className="text-lg font-semibold mb-2 text-neutral-300">Export</h3>
              <div className="flex items-center gap-2">
                <button
                  onClick={handleExportMarkdown}
                  className="px-3 py-1.5 text-sm rounded-md bg-blue-600 hover:bg-blue-500 text-white transition-colors shadow"
                  title="Export as Markdown (.md)"
                >
                  Export .md
                </button>
                <span className="text-xs text-neutral-400">
                  Speichert als: {sanitizeFileName(currentVideo.Title ?? 'Video')}-{sanitizeFileName(currentVideo.VideoID ?? 'Unknown')}.md
                </span>
              </div>
            </div>
                ...
                <div className="mt-3 space-y-3">
                  {currentVideo.DetailedNarrativeFlow && (
                    <button
                      ...
                      aria-label="Neu Transkribieren"
                      disabled={isSaving || isDeleting}
                    >
                      <XCircle size={16} className="mr-2" />
                      {isSaving && !isDeleting ? 'Neu Transkribieren...' : 'Neu Transkribieren'}
                    </button>
                  )}
                  <button
